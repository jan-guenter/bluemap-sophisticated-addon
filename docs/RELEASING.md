...
4. Remove `-SNAPSHOT`, merge the version change through a PR, then create the
   exact annotated tag `v<addon_version>` on the reviewed commit.

The tag workflow reacquires the exact three third-party inputs, runs the same
single authoritative gate, creates SHA-256 checksums, and publishes a GitHub
prerelease plus the matching GitHub Packages Maven publication. The publish
task reuses the already-built outputs and does not repeat compilation. It does
not deploy to any server. Third-party JARs are temporary inputs and never
release assets.
